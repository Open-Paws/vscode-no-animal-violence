...
function scanDocument(document) {
	...
	const severity = SEVERITY_MAP[severityKey] ?? vscode.DiagnosticSeverity.Warning;
...
function activate(context) {
	diagnosticCollection = vscode.languages.createDiagnosticCollection("noAnimalViolence");
	...
	const onConfig = vscode.workspace.onDidChangeConfiguration((event) => {
		if (event.affectsConfiguration("noAnimalViolence")) {
			...
			for (const editor of vscode.window.visibleTextEditors) {
				if (diagnosticCollection) {
					diagnosticCollection.set(editor.document.uri, scanDocument(editor.document));
				}
			}
		}
	});

	context.subscriptions.push(diagnosticCollection, codeActionProvider, onOpen, onChange, onSave, onClose, onConfig);
}
